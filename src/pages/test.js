  ...
  var getBubbleSize = function getBubbleSize(row, col) {
    ...
    if (Math.abs(dx) <= options.xRadius && Math.abs(dy) <= options.yRadius) {
      if (
        ...
        var distToInnerCorner = Math.sqrt(
      ...
      if (
        ...
        var _distToInnerCorner = Math.sqrt(
          Math.pow(Math.abs(dy) - options.yRadius + options.cornerRadius, 2) +
            Math.pow(Math.abs(dx) - options.xRadius + options.cornerRadius, 2)
        );
      ...
      if (isInCornerRegion) {
        var _distToInnerCorner2 = Math.sqrt(
          Math.pow(Math.abs(dy) - options.yRadius + options.cornerRadius, 2) +
            Math.pow(Math.abs(dx) - options.xRadius + options.cornerRadius, 2)
        );
    ...
    var translationMag = options.compact
      ? (options.size - options.minSize) / 2
      : 0;
    var interpolatedTranslationMag = interpolate(
      0,
      options.fringeWidth,
      distanceFromEdge,
      0,
          ...
          rows.map(function (row, i) {
            return /*#__PURE__*/ React.createElement(
              "div",
              {
                ...
                style: {
                  marginTop:
                    i > 0 ? options.size * -0.134 + options.gutter * 0.866 : 0,
                },
              },
